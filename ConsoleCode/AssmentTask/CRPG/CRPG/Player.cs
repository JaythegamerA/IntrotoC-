...
namespace CRPG
{
    public class Player : LivingCreature
    {
        ...
        public Player(string name, int currentHitPoints, int maximumHitPoints, int gold, int experiencePoints, int level, int World) : base(currentHitPoints,maximumHitPoints)
        {
            Name = name;
            Gold = gold;
        ...
        {
            //Check to make sure player has required items
            ...
            if (loc.ItemRequiredToEnter != null)
            {
                ...
                {
                    Console.WriteLine($"{Name}, you cannot proceed without possessing at least one {loc.ItemRequiredToEnter.Name}.");
                    return;
                }
            }
        ...
        public void MoveNorth()
        {
            ...
            {
                Console.WriteLine("Nothing is there for you.");
            }
        }
        ...
        {
            if (CurrentLocation.LocationTP != null)
            {
                Console.Clear();
                Console.WriteLine("Your medallion opens a portal, a short pink tunnel into another world. You dive through and the portal carries " +
                    "you along, until you land on the other side, in a strange new location, where you find a new weapon.");
                world = 1;
                Console.WriteLine("Welcome to Bjork.");

                InventoryItem sword = new InventoryItem(World.ItemByID(World.ITEM_ID_RUSTY_SWORD), 1);
                Inventory.Add(sword);
                MoveTo(CurrentLocation.LocationTP);
            } else
            {
                Console.WriteLine((world == 0) ? "The medallion does not react to your thoughts." : "You aren't welcome in your world anymore. Maybe this world " +
                    "is more hospitable. In any case, you cannot risk going back.");
            }
        }
